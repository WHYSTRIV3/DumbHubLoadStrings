...
local ui = library:CreateWindow("Anime Power Tycoon")
...
function getTycoons()
    ...
    for _,v in pairs(WS.tycoon1.Tycoons:GetChildren()) do
...
Main:CreateDropdown("Select Tycoon", getTycoons(), function(Tycoon)
    SelectedTycoon = Tycoon

end)


Main:CreateButton("Teleport To Tycoon", function()
    if SelectedTycoon then
        Player.Character.HumanoidRootPart.CFrame = WS.tycoon1.Tycoons:FindFirstChild(SelectedTycoon).Spawn.CFrame
    end
end)

Main:CreateToggle("Auto Collect", true, function()
    if SelectedTycoon then
        local giver = WS.tycoon1.Tycoons[SelectedTycoon].Essentials.Collector1.Giver
        giver.Size = Vector3.new(.1, .1, .1)
...
Main:CreateToggle("Auto Buy", true, function()
    for _,v in pairs(WS.tycoon1.Tycoons[SelectedTycoon].Buttons:GetChildren()) do
        if not v.Buton:FindFirstChild("Resim") then
       v.Buton.Size = Vector3.new(.1, .1, .1)
        v.Buton.CanCollide = false
        v.Buton.CFrame = Player.Character.HumanoidRootPart.CFrame
        task.wait(0.1)
        v.Buton.CFrame = Player.Character.HumanoidRootPart.CFrame * CFrame.new(-1000, -1000, 10000000)
        end
    end
end)

Main:CreateToggle("Auto Rebirth", true, function()
    RS.Event.rebirth:FireServer()
end)
