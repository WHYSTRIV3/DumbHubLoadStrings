...
local ui = library:CreateWindow("Farming Tycoon")

local Main = ui:new("Moneyy")
local Egg = ui:new("Teleport")
...
--Main
Main:CreateToggle("Unlimited Money", true, function()
    RS.Remote.Event.Offline["[S-C]TryGetFreeReward"]:FireServer()
end)
--Dont Work
Main:CreateToggle("Auto Research", true, function()
    RS.Remote.Event.Research["[C-S]tryUpgradeCommonResarch"]:FireServer()
end)

Main:CreateToggle("Auto Upgrade Animals", true, function()
    RS.Remote.Event.Spawn["[C-S]TryUpgrade"]:FireServer() 
    RS.Remote.Event.Spawn["[C-S]TryRebirth"]:FireServer() 
end)
