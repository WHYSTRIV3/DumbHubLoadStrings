...
function getOwner()
    for _,v in pairs(WS["Zednov's Tycoon Kit"].Tycoons:GetChildren()) do
		if tostring(v.Owner.Value) == Player.Name then
			SelectedTycoons = v.Name
		end
	end
end
...
Main:CreateLabel("", "Choosen Tycoon: "..SelectedTycoons)


Main:CreateDropdown("Selected Tycoon", getTycoons(), function(Tycoons)
    SelectedTycoons = Tycoons
    print(SelectedTycoons)
end)


Main:CreateToggle("Auto Buy", true, function()
    for _,v in pairs(WS["Zednov's Tycoon Kit"].Tycoons[SelectedTycoons].Buttons.Level1:GetChildren()) do
...
Main:CreateToggle("Auto Collect", true, function()
        local giver = game:GetService("Workspace")["Zednov's Tycoon Kit"].Tycoons.Blue.Essentials.Giver
...
Main:CreateButton("Make Shop invisible", function()
game:GetService("Players").WHYSTRlVE.PlayerGui.UI.Store.Visible = false
end)
